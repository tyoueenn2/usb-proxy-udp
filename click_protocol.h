...
constexpr uint8_t VERSION=VERSION1;
...
struct Request {
    ...
    uint8_t version=VERSION1;
};
inline bool parse(const uint8_t* p,size_t n,Request& r) {
    if(n!=REQUEST_SIZE||std::memcmp(p,"UPC1",4)||(p[4]!=VERSION1&&p[4]!=VERSION2)||p[7]||get32(p+36))return false;
    if(p[5]!=uint8_t(Operation::Schedule)&&p[5]!=uint8_t(Operation::ReleaseAll))return false;
    r.operation=Operation(p[5]);r.button=p[6];r.session=get64(p+8);r.command=get64(p+16);r.version=p[4];
...
inline std::array<uint8_t,ACK_SIZE> packet(const Ack& a) {
    std::array<uint8_t,ACK_SIZE> p{};std::memcpy(p.data(),"UPA1",4);p[4]=a.version;p[5]=uint8_t(a.status);p[6]=a.button;
...
inline bool parse_ack(const uint8_t* p,size_t n,Ack& a) {
    if(n!=ACK_SIZE||std::memcmp(p,"UPA1",4)||(p[4]!=VERSION1&&p[4]!=VERSION2)||p[7]||get16(p+42)||get32(p+44))return false;
    if(p[5]<uint8_t(Status::Accepted)||p[5]>uint8_t(Status::StaleCommand))return false;
    a.status=Status(p[5]);a.button=p[6];a.session=get64(p+8);a.command=get64(p+16);a.version=p[4];
