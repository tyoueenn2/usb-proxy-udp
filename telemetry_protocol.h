...
struct Snapshot {
    ...
    uint32_t poll_us=0,physical_age_us=0xffffffffu,physical_received=0,physical_submitted=0;
    uint16_t output_queue=0,synthetic_pending=0;
    uint32_t superseded=0,writer_failures=0;
    ...
    bool operator==(const Snapshot& b)const {
        return ready==b.ready&&physical==b.physical&&persistent==b.persistent&&scheduled==b.scheduled&&
            ...
            poll_us==b.poll_us&&physical_received==b.physical_received&&
            physical_submitted==b.physical_submitted&&output_queue==b.output_queue&&
            synthetic_pending==b.synthetic_pending&&superseded==b.superseded&&
            writer_failures==b.writer_failures&&cumulative_x==b.cumulative_x&&cumulative_y==b.cumulative_y;
    }
};
struct Subscription {
    ...
    bool accept(const uint8_t* p,size_t n,uint64_t now) {
        if(n!=24||(std::memcmp(p,"UPS1",4)&&std::memcmp(p,"UPS2",4)&&std::memcmp(p,"UPS3",4))||get32(p+4))return false;
    ...
    std::array<uint8_t,128> packet3(uint64_t server,const Snapshot& s) {
        ...
        put64(p.data()+24,token);put32(p.data()+32,sequence++);put32(p.data()+36,uint32_t(s.xmin));
        put32(p.data()+40,uint32_t(s.xmax));put32(p.data()+44,uint32_t(s.ymin));
        put32(p.data()+48,uint32_t(s.ymax));put32(p.data()+52,s.poll_us);put64(p.data()+56,s.generation);
        put64(p.data()+64,s.snapshot_ns);put64(p.data()+72,uint64_t(s.cumulative_x));
        put64(p.data()+80,uint64_t(s.cumulative_y));put32(p.data()+88,s.physical_age_us);
        put32(p.data()+92,s.accepted);put32(p.data()+96,s.completed);
        click_protocol::put16(p.data()+100,s.active);click_protocol::put16(p.data()+102,s.queued);
        put32(p.data()+104,s.physical_received);put32(p.data()+108,s.physical_submitted);
        click_protocol::put16(p.data()+112,s.output_queue);click_protocol::put16(p.data()+114,s.synthetic_pending);
        put32(p.data()+116,s.superseded);put32(p.data()+120,s.writer_failures);return p;
    }
};
}
